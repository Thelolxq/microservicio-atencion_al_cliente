...
export class SupportTicketController {
  ...
  async getById(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (ticket) {
        ...
        if (error instanceof Error) {
          if(error.message.includes('formato de UUID')){
             res.status(400).json({ message: `Formato de ID inválido: ${error.message}` });
          } else {
             res.status(500).json({ message: 'Error interno del servidor' });
          }
        } else {
  ...
  async addMessage(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!authorId || !content || !ticketId) {
        res.status(400).json({ message: 'ticketId, authorId y content son requeridos.' });
        return;
      }
